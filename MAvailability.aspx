<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MAvailability.aspx.cs" Inherits="BenhurstV3.MAvailability" %>
...
alt="Benhurst"/></header>
    ...
    <div id="logoutlink">
        ...
        <fieldset>
                <legend>Select a session to Accept or reject a player from a session</legend>
                <label>
                Select a Session
         ...
         <h3>Players Available for the session</h3>
                <div class="table-container">
            <p>
            <asp:GridView ID="session_list" runat="server"  AutoGenerateColumns="False" DataKeyNames="AvailabilityID, PlayerRegistrationNo" OnRowEditing="ses_list_RowEditing" OnRowUpdating="ses_list_RowUpdating" OnPageIndexChanging="ses_list_PageIndexChanging" OnRowCancelingEdit="ses_list_RowCancelingEdit">
                <Columns>
                    ...
                    <asp:BoundField DataField="Decision" HeaderText="Is the Player Available?" SortExpression="Decision" ReadOnly="true"></asp:BoundField>
                    <asp:BoundField DataField="Acceptance" HeaderText="Accept the Player onto the Session (yes/no)" SortExpression="Acceptance"></asp:BoundField>
                    <asp:CommandField ShowEditButton="true" />
                </Columns>
            </asp:GridView>
                </p>

            <asp:SqlDataSource runat="server" ID="PAvaconn" ConnectionString='<%$ ConnectionStrings:BenhurstConnectionString %>' SelectCommand="SELECT Availability.AvailabilityID, Availability.SessionID, Availability.PlayerRegistrationNo, users.Forename, users.Surname, Availability.Decision, Availability.Acceptance FROM Availability INNER JOIN users ON Availability.PlayerRegistrationNo = users.PlayerRegistrationNo"></asp:SqlDataSource>
        </div>
